        ...
        <section id="projects" class="py-8 sm:py-12 lg:py-16 relative bg-white/50 backdrop-blur-sm border-y border-brand-latte/30 scroll-mt-20">
            <div class="max-w-7xl mx-auto px-6 sm:px-12 lg:px-20">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-5 sm:gap-6">
                    ...
                    <div class="portfolio-card bg-white border border-brand-latte/30 shadow-sm hover:shadow-lg transition-all duration-400 overflow-hidden group flex flex-col">
                        {{-- Image --}}
                        <div class="overflow-hidden relative">
                            @if(Str::startsWith($project->image_url, ['http://', 'https://']))
                                <img src="{{ $project->image_url }}" alt="{{ $project->title }}" class="w-full h-44 object-cover group-hover:scale-105 transition-all duration-600">
                            @else
                                <img src="{{ asset($project->image_url) }}" alt="{{ $project->title }}" class="w-full h-44 object-cover group-hover:scale-105 transition-all duration-600">
                            @endif
                            {{-- Category badge overlay --}}
                            <span class="absolute top-3 left-3 px-2.5 py-1 text-[10px] font-bold tracking-widest uppercase bg-white/90 backdrop-blur-sm rounded-full text-brand-espresso shadow-sm border border-brand-latte/30">{{ $project->category }}</span>
                        </div>
                        {{-- Info --}}
                        <div class="p-5 flex flex-col flex-grow">
                            <span class="inline-block mb-2 text-[10px] font-semibold tracking-widest uppercase text-brand-terracotta">{{ $project->tech_stack }}</span>
                            <h3 class="text-base font-bold font-serif text-brand-espresso mb-2 leading-snug">{{ $project->title }}</h3>
                            <p class="text-brand-mocha text-xs leading-relaxed flex-grow mb-4">{{ Str::limit($project->description, 120) }}</p>
                            <a href="{{ $project->github_link ?? '#' }}" target="_blank" class="inline-flex items-center gap-2 text-xs font-bold text-brand-espresso hover:text-brand-terracotta transition-colors mt-auto group/link">
                                <svg class="w-3.5 h-3.5" fill="currentColor" viewBox="0 0 24 24"><path fill-rule="evenodd" clip-rule="evenodd" d="M12 2C6.477 2 2 6.477 2 12c0 4.42 2.865 8.166 6.839 9.489.5.092.682-.217.682-.482 0-.237-.008-.866-.013-1.7-2.782.603-3.369-1.34-3.369-1.34-.454-1.156-1.11-1.462-1.11-1.462-.908-.62.069-.608.069-.608 1.003.07 1.531 1.03 1.531 1.03.892 1.529 2.341 1.087 2.91.831.092-.646.35-1.086.636-1.336-2.22-.253-4.555-1.11-4.555-4.943 0-1.091.39-1.984 1.029-2.683-.103-.253-.446-1.27.098-2.647 0 0 .84-.269 2.75 1.025A9.578 9.578 0 0112 6.836c.85.004 1.705.114 2.504.336 1.909-1.294 2.747-1.025 2.747-1.025.546 1.377.203 2.394.1 2.647.64.699 1.028 1.592 1.028 2.683 0 3.842-2.339 4.687-4.566 4.935.359.309.678.919.678 1.852 0 1.336-.012 2.415-.012 2.743 0 .267.18.578.688.48C19.138 20.161 22 16.418 22 12c0-5.523-4.477-10-10-10z"></path></svg>
                                View on GitHub
                                <svg class="w-3 h-3 group-hover/link:translate-x-1 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M9 5l7 7-7 7"/></svg>
                            </a>
                        </div>
                    </div>
                    @endforeach
